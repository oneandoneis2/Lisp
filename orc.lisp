  ...
  (let ((x (read)))
    (if (not (and (integerp x) (>= x 1) (<= x *monster-num*)))
      ...
      (let ((m (aref *monsters* (1- x))))
        (if (monster-dead m)
          (progn (pinc "That monster is already dead.")
                 (pick-monster))
          m)))))
...
(defmethod monster-hit ((m hydra) x)
  (decf (monster-health m) x)
  (if (monster-dead m)
    (princ "The corpse of the headless hydra falls to the floor!")
...
(defmethod monster-attack ((m slime-mold))
  (let ((x (randval (slime-mold-sliminess m))))
    ...
    (when (zerop (random 2))
      (princ "It squirts in your face, taking away a health point! ")
      (decf *player-health*))))
